...
class GCNAbsaModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        h1, h2, adj_ag, adj_s, pooled_output = self.gcn(inputs)
        ...
        return outputs1, outputs2, adj_ag, adj_s, pooled_output
...
class GCNBert(nn.Module):
    ...
    def forward(self, inputs): 
        ...
        for i in range(self.layers):
            ...
            Ax = torch.matmul(weight_adj, gcn_outputs)     # [batch_size, attention_heads, maxlen, attdim]
            Ax = Ax.mean(dim=1)  # [batch_size, maxlen, attdim]
  
            Ax = self.W(Ax)   # [batch_size, maxlen, attdim]
            weights_gcn_outputs = F.relu(Ax)  # [batch_size, maxlen, attdim]

            gcn_outputs = weights_gcn_outputs     
            layer_list.append(gcn_outputs)
            gcn_outputs = self.gcn_drop(gcn_outputs) if i < self.layers - 1 else gcn_outputs # [batch_size, maxlen, maxlen]

            weight_adj=weight_adj.permute(0, 2, 3, 1).contiguous()  # [batch_size, maxlen, maxlen, heads]
            node_outputs1 = gcn_outputs.unsqueeze(1).expand(batch, len, len, self.attdim)  # [batch_size, maxlen, maxlen, attdim]
            node_outputs2 = node_outputs1.permute(0, 2, 1, 3).contiguous()  # [batch_size, maxlen, maxlen, attdim]
            node = torch.cat([node_outputs1, node_outputs2], dim=-1)  # [batch_size, maxlen, maxlen, attdim*2]
            edge_n=torch.cat([weight_adj, node], dim=-1)  # [batch_size, maxlen, maxlen, attdim*2+heads]
            edge = self.Wx(edge_n)  # [batch_size, maxlen, maxlen, heads]
            edge = self.gcn_drop(edge) if i < self.layers - 1 else edge
            weight_adj=edge.permute(0,3,1,2).contiguous()  # [batch_size, heads, maxlen, attdim]


        # outputs = torch.cat(layer_list, dim=-1)  # [batch_size, maxlen, attdim*[(num_layers)+1]]
        # node_outputs = self.aggregate_W(outputs)
        # node_outputs=self.Wi(gcn_outputs)
        outputs = torch.cat([gcn_output.unsqueeze(0) for gcn_output in layer_list], dim=0)
        node_outputs = outputs.mean(dim=0)
